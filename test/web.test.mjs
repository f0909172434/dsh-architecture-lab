...
test('daily API accepts bounded Unicode drafts, holds paid starts and rejects unauthenticated writes',()=>fixture(async({root,origin,call,launches})=>{
  const post=(input,headers={})=>call('/api/architecture-lab/daily-action',{method:'POST',headers:{origin,'content-type':'application/json',...headers},body:JSON.stringify(input)})
  const input={action:'import',label:'Notes <script>unsafe</script>',source:''}
  assert.equal((await post(input,{origin:'https://example.invalid'})).status,403)
  assert.equal((await post(input,{'x-architecture-lab-token':''})).status,403)
  const item=await (await post(input)).json();assert.ok(item.id)
  const prompt='繁體中文任務。'.repeat(600)
  const response=await post({action:'task',projectId:item.id,recipe:'D',prompt})
  assert.equal(response.status,200);const job=await response.json();assert.equal(job.prompt,prompt)
  assert.equal((await post({action:'start',jobId:job.id})).status,400)
  assert.equal((await post({action:'resume-check',jobId:'../../private'})).status,400)
  assert.equal((await post({action:'task',projectId:item.id,prompt:'a'.repeat(70000)})).status,400)
  const state=await (await call('/api/architecture-lab/daily')).json()
  assert.equal(state.jobs.length,1);assert.equal(state.runs.length,0);assert.equal(state.paidReady,false)
  assert.equal(readRegistry(root).runs.length,0);assert.deepEqual(launches,[])
  assert.equal((await call('/architecture-lab/daily.mjs')).status,200)
  assert.equal((await post({action:'check',recipe:'B'})).status,200)
  assert.equal(launches[0][0],'daily-run-check')
  updateRegistry(join(root,'daily'),s=>{s.active={runId:'one',pid:process.pid,control:{token:'private-daily-control'}}})
  assert.ok(!(await (await call('/api/architecture-lab/daily')).text()).includes('private-daily-control'))
}))
