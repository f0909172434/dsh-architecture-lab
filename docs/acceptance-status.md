...
| DSH entry, recipe selection, start/stop, reports and export | Actual DSH server/client-module wiring, authenticated API operations and external-tool checks. [Panel](web-panel.md). | **Partial:** rendered layout, keyboard use, navigation and actual user interaction remain pending. HTTP success is not GUI acceptance. |
| Cancellation, restart and explicit recovery | Real controller SIGKILL, independent guest cleanup, preserved records/partial files and explicit resumes. Original deadline and cumulative request identity are checked. [Management](management.md), [validation](dev9-validation.json). | **Verified offline:** the corresponding graphical workflow still needs [acceptance](gui-acceptance.md). |
| Useful daily recipe mode | Separate projects/jobs, custom drafts, output review/adoption/export and per-project memory pass scripted real-tool checks. [Daily mode](daily-mode.md). | **Partial:** arbitrary user tasks have not been accepted with a real model; paid use remains held. Contents-only copying, file limits and no general container network are documented constraints. |
...
1. Complete the [graphical checklist](gui-acceptance.md) through permitted tool
   access or a clearly identified human operator. Existing administrative browser
   restrictions remain in effect; CLI/HTTP checks do not bypass or satisfy them.
