...
experience retrieval, not natural memory accumulation.

The agent-visible contracts in `tasks/catalog.mjs` govern these fixed tasks:

| Task | Contract and limited interpretation |
| --- | --- |
| debug-amount | Finite numeric input or decimal text, optional sign, whitespace and valid three-digit comma groups; invalid/empty text and other types return null. |
...
| stale-date | Strict YYYY-MM-DD input, valid Gregorian date, UTC-midnight milliseconds; invalid input returns null. Fixed probes use modern years. |
...
to cover repeated hyphens already present in the fixed judge. Neither judge
values nor historical pilot records changed. Finite cases do not prove general
...
provider load, cache or time trends. Stop at the budget boundary; never
...
do not reset it. Cache use is priced explicitly; incomplete usage retains its
reservation. The prompt states the allowance; the broker does not enforce a
