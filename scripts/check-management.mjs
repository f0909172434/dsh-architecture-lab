...
assert.ok(report.runs.every(row=>row.evidenceAudit.integrityVerified),'every completed or cancelled attempt must retain a verified evidence seal')
assert.ok(report.runs.every(row=>!row.evidenceValid),'offline evidence never qualifies as research')
